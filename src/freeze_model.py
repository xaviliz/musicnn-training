...
if __name__ == '__main__':
    ...
    for model in models:
        ...
        if config['model_number'] == 2:
            ...
            raise Exception('Model number not found ({})'.format(config['model_number']))

        # parser source task:
        if 'MSD' in config['load_model']:
            source_task = 'msd'
        elif 'MTT' in config['load_model']:
            source_task = 'mtt'
        elif 'audioset' in config['load_model']:
            source_task = 'audioset'
        else:
            raise Exception('Source task not found in "{}"'.format(config['load_model']))


        output_graph = os.path.join(model_fol, '{}-{}-{}.pb'.format(config['audio_rep']['identifier'], arch, source_task))
        print(output_graph)
        ...
        with graph.as_default():
            ...
            [x, y_, is_train, y, normalized_y, cost, model_vars] = train.tf_define_model_and_cost_freeze(config)
