...
if __name__ == '__main__':
    ...
    to = open(experiment_folder + 'experiment_{}.result'.format(FOLD), 'w')
    to.write('Experiment: ' + str(models))
    to.write('\nAUC: ' + str(roc_auc))
    to.write('\nAUC: ' + str(pr_auc))
    to.close()

    predictions = ['{}\t{}'.format(id, pred) for id, pred in zip(id_array, pred_array)]
    predictions_file = experiment_folder + 'results_{}.result'.format(FOLD)
